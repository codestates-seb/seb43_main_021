...
export default function MyAuction() {
  ...
  const completedAuctionCount =
    countCompletedAuctionData > 0 ? (
      <span>{countCompletedAuctionData}</span>
    ) : null;
  return (
    <Wrapper>
      <TopContainer>
        ...
        <MainTitle>
          나의 경매내역
          <WritingButton>
            <StyledLink to="/createauction">글쓰기</StyledLink>
          </WritingButton>
          <VscAccount />
        </MainTitle>
        <BottomSelector>
          <FirstButton
            isselected={auction ? "true" : "false"}
          ...
          <LastButton
            isselected={!auction ? "true" : "false"}
            onClick={auctionCompletedHandler}
          >
            경매완료{completedAuctionCount}
          </LastButton>
        </BottomSelector>
      </TopContainer>
    </Wrapper>
...
const FirstButton = styled.div`
  ...
  cursor: pointer;
  border-bottom: ${(props) =>
    props.isselected === "true" ? "2px solid black" : "none"};
  span {
    margin-left: 0.25rem;
  }
  @media only screen and (min-width: 768px) {
